 ...
 *        Jeffrey Song
 *
 *  Project Implemented:    Yes
 *  Extra Credit Attempted: Yes
 *
 *  Performance Index W/O EC:   0.9
 *  Performance Index With EC:  0.47
 *-------------------------------------------------------------------- */
...
void* mm_malloc (size_t size) {
  ...
  ptrFreeBlock = searchFreeList(reqSize); // get free block

  if(ptrFreeBlock == NULL){

    requestMoreSpace(reqSize); // if not enough room, get more heap space
    ptrFreeBlock = searchFreeList(reqSize); // look for free block

  }

  removeFreeBlock(ptrFreeBlock); // Remove free block

  blockSize = SIZE(ptrFreeBlock->sizeAndTags); // get block size

  precedingBlockUseTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED; // Store the preceding block's used tag

  size_t MIN_BLOCK_DIFFERENCE = blockSize - reqSize;

  if (MIN_BLOCK_SIZE > MIN_BLOCK_DIFFERENCE) { // split block if MIN_BLOCK_SIZE > (blockSize - reqSize)

    BlockInfo* followingBlock = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize); // get pointer to next block

    followingBlock->sizeAndTags |= TAG_PRECEDING_USED; // Set the next block's previous used tag
    ...
    ptrFreeBlock->sizeAndTags |= TAG_USED; // Set the used tag of the block
    *((size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize)) = TAG_PRECEDING_USED | MIN_BLOCK_DIFFERENCE; // set preceiding used tag of free block
    *((size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize - WORD_SIZE)) = TAG_PRECEDING_USED | MIN_BLOCK_DIFFERENCE; // set preceding used tag within boundary

    insertFreeBlock((BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize)); // insert free block
  }
...
void mm_free (void *ptr) {
  ...
  blockInfo = (BlockInfo*) UNSCALED_POINTER_SUB(ptr, WORD_SIZE);              // Copy current block data into blockInfo
  blockInfo->sizeAndTags &= (~TAG_USED);                                      // Set current block's used tag to 0
  
  payloadSize = SIZE(blockInfo->sizeAndTags);                                 // Get block size
  
  followingBlock = (BlockInfo*) UNSCALED_POINTER_ADD(blockInfo, payloadSize); // Copy next block data
  followingBlock->sizeAndTags &= (~TAG_PRECEDING_USED);                       // Set next block's preceding used tag to 0

  // Update boundary tag
  boundaryTag = UNSCALED_POINTER_ADD(blockInfo, payloadSize - WORD_SIZE);     // Obtain tag address
  *boundaryTag = blockInfo->sizeAndTags;  // Copy to header to footer
...
void* mm_realloc(void* ptr, size_t size) {
  ...
  if (ptr == NULL) {      // Condition, if ptr is null, return malloc
    return mm_malloc(size);
  }

  if (size == 0) {        // Condition, if size is 0, return 0 
    mm_free(ptr);
    return 0;
  }
  
  oldBlockInfo = (BlockInfo *) UNSCALED_POINTER_SUB(ptr, WORD_SIZE);    // Copy current block data to new block
  oldsize = SIZE(oldBlockInfo->sizeAndTags);                            // Obtain size
  if (size < oldsize){                                                 
    oldsize = size;
  }
  ...
  // Manual memcpy
  char *chptr = (char *) ptr;               
  char *chnewptr = (char *) newptr;         

  for(int i = 0; i < oldsize; i++){   // Copy old block to new block
    chnewptr[i] = chptr[i];
  }
