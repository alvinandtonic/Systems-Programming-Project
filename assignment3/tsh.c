...
*/
void eval(char *cmdline) {
    int val;     // Return value for builtin_cmd
    int bg;         // Value for whether if it is bg or fg using parseline function                     
    ...
    bg=parseline(cmdline,argv);         // Check if bg or fg 
    val=builtin_cmd(argv);           // Load value
    
    if(val==0){                      // If it isn't a builtin_cmd
	    pid_t pid;
	    ...
	    if((pid=fork())==0){
            ...
            setpgrp();                                  // Make child the group leader
            execvp(argv[0],argv);                       // Execute user command if successful
        ...
        // Parent
            ...
            sigprocmask(SIG_UNBLOCK, &set, NULL);       // Unblock SIGCHILD signal
            if(bg)
...
void sigint_handler(int sig) 
{
    int pid = fgpid(jobs);  // inititalize pid as the process ID 
    int jid = pid2jid(pid);  // convert pid to jid

    if (pid > 0) {   // run if pid exists 

        kill(-pid, SIGINT);  // kill signal 
        printf("Job [%d] (%d) terminated by signal %d\n", jid, pid, sig);  //signal termination
