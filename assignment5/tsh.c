...
void eval(char *cmdline) {
    ...
    if(builtin_cmd(argv) == 0) {
        ...
        if((pid0 = fork()) == 0) {  
            ...
            // Parsing input and args
            char *args[MAXARGS][MAXARGS];
            int row = 0;
            int col = 0;
            
            // Piping prep variables
            int numPipes = 0;
            int i = 0;
            ...
            if( numPipes >= 1) {        // Execute piping
                ...
                while(i < numPipes){    // Create pipes
                    pipe(pipeArray[i]);
                    i++;
                }
                
                for(int i = 0; i <= (numPipes); i++) {  // Piping
                    pid_t pid;
